...
const db = mysql.createConnection({
    host: "bkkr1bxcndmwsocy1jcq-mysql.services.clever-cloud.com",
    user: "uipiagwszcpk2wwz",
    password: "TY78F9VwDjDI7quD7KJe",
    database: "bkkr1bxcndmwsocy1jcq"
});
...
app.get('/getVaccinationCenters', (req, res) => {
    const sql = "SELECT id, name, location, dosageDetails, timings, availableSlots FROM vaccination";
...
app.post('/addVaccinationCenter', (req, res) => {
    const sql = "INSERT INTO vaccination (name, location, dosageDetails, timings, availableSlots) VALUES ?";
    const values = [
...
app.post('/removeVaccinationCenter', (req, res) => {
    const { id } = req.body;
    const sql = 'DELETE FROM vaccination WHERE id = ?';
...
app.post('/bookVaccinationCenter', (req, res) => {
    const { id } = req.body;
    const sqlSelect = 'SELECT availableSlots FROM vaccination WHERE id = ?';
    const sqlUpdate = 'UPDATE vaccination SET availableSlots = ? WHERE id = ?';
